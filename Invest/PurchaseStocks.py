...
def toBuy(self, current, fiveYearAverage, fiveYearMinimum, fiveYearMaximum, recentMaximum):
    BUYING_RANGE_VS_MAX = .6 #percent of max when we're willing to purchase

    MinVsAvgDifference = abs(fiveYearMinimum - fiveYearAverage)
    MaxVsAvgDifference = abs(fiveYearMaximum - fiveYearAverage) 

    if(MaxVsAvgDifference >= MinVsAvgDifference*2 or current <= (recentMaximum * .6)):
        True

def toSell(slef, purchasePrice, current, fiveYearAverage, fiveYearMinimum, fiveYearMaximum):
    pass

def buy(self, parameter_list):
    #purchase stock with TD Ameritrade "Place Order" API
    {
  "orderType": "MARKET",
  "session": "NORMAL",
  "duration": "DAY",
